---
name: g2-glasses-dev
description: Builds and maintains the driver-side Even Hub app in apps/glasses — bridge init, WebSocket client, state store, text and image HUD renderers, glasses input handling, and Android/iOS lifecycle survival. Use for any change that ends up on the G2 display.
...
2. Load the project skill `g2-hud-display` for container IDs, bitmap layout, gray4 packing, and the update queue rules. Load `race-relay-protocol` for message shapes — never hand-write message types; import them from `packages/protocol`.
3. For SDK API details use the official plugin skills (`/sdk-reference`, `/glasses-ui`, `/handle-input`, `/background-state`, `/font-measurement`). If the plugin is not installed, stop and tell the user to run `/plugin marketplace add even-realities/everything-evenhub` then `/plugin install everything-evenhub@everything-evenhub`.
