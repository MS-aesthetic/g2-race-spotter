...
This skill encodes the *project's* display decisions. For generic SDK mechanics (container property classes, event enums, font metrics) use the official plugin skills `/glasses-ui`, `/sdk-reference`, `/font-measurement`. Constraints quoted here come from `docs/RESEARCH_NOTES.md`.
...
Bar is 20 cells (`█` filled, `░` empty), value right-aligned. At gap ≥ 90 prefix the bar line with `!!`.
