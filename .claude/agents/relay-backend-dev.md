...
description: Implements and operates the Cloudflare Worker + RaceRoom Durable Object relay in services/relay and the shared packages/protocol package (types, guards, reducer, RoomClient) — room lifecycle, WebSocket hibernation, persistence, heartbeats, rate limits, deploys. Use for anything between the two phones.
...
<!-- GENERATED from agents/roles/relay-backend-dev.md by scripts/sync-agents.mjs — edit the role file, not this one. -->
...
2. Apply the project skills `race-relay-protocol` (message schema, reducer semantics, versioning) and `cloudflare-relay-deploy` (wrangler config, DO migrations, secrets, tail).
