...
In: bridge init, phone companion page (join form, status, log panel), `RoomClient` wiring, state store, render-mode selection, text renderer, `glyphs.ts`, update queue (text path), status strip, input mapping, exit dialogue.
Out: image renderer (050), Android background hardening (060).
...
R2. MUST call `createStartUpPageContainer` once; in text mode slot 2 is a text container.
...
| AC-6 | Given `?render=text`, the stored override, or simulator detection, then startup builds the page with a text container in slot 2 and never creates an image container | `apps/glasses/test/render-mode.test.ts` |
| AC-7 | Given the simulator running `dev:sim`, when `fake-spotter --scenario lanes` and `gap-sweep` run, then screenshots show the expected symbol and bar at each step | `[HW]` simulator screenshots `qa/<date>/030-*.png` |
