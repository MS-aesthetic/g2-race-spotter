...
| 53 | 2026-09-25 | T055 | g2-glasses-dev | review-blocked ×2 → done | block → block → approve with nits | b1dbb78 | Design round 3 (protocol v2 `cars[3]`, relay 6 s stale clear, half-size icons, three bars, three segment rows). Lessons: a "quiet for N s" window must be keyed on the party that is supposed to be quiet (`calledAt`, not `updatedAt`); a protocol version bump must validate stored DO state or a redeploy bricks live rooms. |
| 54 | 2026-09-25 | T056 | spotter-pwa-dev | done | approve with nits | 7c6a0e2 | Design round 4 spotter side: room presets in DO state (additive), 24 h TTL, random room + PIN join, ▼▬▲ row + vertical 3-segment sliders, icon header, preset buttons. Saving messages to a room was a small change because the room already persists state. |
| 55 | 2026-09-25 | T057 | g2-glasses-dev | failed → done | block → approve with nits | ff4c4ab | Worker correctly stopped: SDK caps 4 image containers/page, six were planned; re-scoped to four 288×48 edge strips with seam-straddling centre items. Block: skip-unchanged must compare only with the last successful send. |
