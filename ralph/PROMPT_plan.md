...
3. Write `IMPLEMENTATION_PLAN.md` from scratch in the role file's format: spec focus = the lowest-numbered spec with an unmet non-`[HW]` criterion; tasks one-iteration sized, each with owner, spec + AC, and named tests; `[HW]` items under *Needs human*. Mark only genuinely independent tasks parallel-ready, with stream/lock metadata, non-overlapping write scopes, and explicit dependency/integration gates in the active lease table; the coordinator records exact base/worktree values at dispatch.
