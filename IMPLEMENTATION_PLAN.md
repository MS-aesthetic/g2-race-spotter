# Implementation plan — 2026-09-03T16:16:38-04:00
Status: BUILDING
...
|---|---|---|---|---|---|
| L004R | T004b | protocol-manifest-repair | `packages/protocol/package.json`; `packages/protocol/src/index.ts`; `packages/protocol/tsconfig.json`; `packages/protocol/test/package-resolution.test.ts` | `protocol-manifest` | exact base `42d7d5b8e81eea5266833bf584edf9c491c485ed`; repair quarantined candidate `20f5f7c`; review the combined exact base-to-repair-head range, then integrate only after approval and full root gates |

L001/T002h, L002/T004, L003/T005, and L003R/T005a are retired after approved serial integration. L004/T004a is retired and its candidate is quarantined after review `block`; L004R/T004b is the only active lease at this dependency boundary.

## Next (ordered; the serial runner takes the first unchecked task)
- [ ] T004b (owner: relay-backend-dev) (spec: 020 AC-1) [stream: protocol-manifest-repair; lease: L004R; lock: protocol-manifest; exact base: `42d7d5b`; repairs: quarantined T004a candidate `20f5f7c`] Repair the blocking findings: "manifest points to `src/index.ts` but `index` re-exports nonexistent `./reduce.js` under `noEmit`; plain Node22 bare import fails, Vitest masked it." Keep the package private/workspace-only; use a source TypeScript export graph whose internal reducer specifier is `./reduce.ts`, enable `allowImportingTsExtensions` in `packages/protocol/tsconfig.json`, retain the source entry manifest, and replace the masked test with a child `process.execPath --input-type=module` invocation from a workspace-consumer context that bare-imports and asserts both a guard and the reducer. Change only `packages/protocol/package.json`, `packages/protocol/src/index.ts`, `packages/protocol/tsconfig.json`, and `packages/protocol/test/package-resolution.test.ts`; do not claim packed-install or `--preserve-symlinks` support. Run protocol tests and every root gate; review the combined `42d7d5b..repair-head` range before serial integration.
- [ ] T004a (owner: relay-backend-dev) (spec: 020 AC-1) [quarantined after review `block`; do not dispatch] Add explicit Node 22 ESM and TypeScript package entry metadata and prove a workspace consumer's bare package import; candidate `20f5f7c` remains unintegrated until T004b repairs its exact findings.
- [ ] T006 (owner: relay-backend-dev) (spec: 020 AC-8) [stream: protocol-client; lock: protocol-client; depends: T004b integrated; may run concurrently with T007] Implement the injected-WebSocket `RoomClient` in `packages/protocol/src/client.ts`, export wiring in `packages/protocol/src/index.ts`, and `packages/protocol/test/client.test.ts` with hello, ping, reconnect/backoff, per-open sequence reset, replay gating, `lastFrameAt`, and disconnected-intent coalescing; verify protocol workspace plus root gates.
- [ ] T007 (owner: relay-backend-dev) (spec: 020 AC-3) [stream: relay; lock: relay-room; depends: T004b integrated; may run concurrently with T006] Build the Worker and SQLite-hibernating `RaceRoom` within `services/relay/**` only, with URL/hello validation, immediate state replay, persisted full-state reduction, and broadcasts; verify `services/relay/test/roundtrip.test.ts` against self-managed `wrangler dev` plus root gates.
...
- T004a candidate `20f5f7c` is quarantined and not integrated: its source manifest targets an export graph containing `./reduce.js`, which does not exist under `noEmit`, while its Vitest test masks the plain-Node failure.
- T004b is deliberately private/workspace-only: source TypeScript exports plus a spawned plain-Node workspace-consumer test prove the supported bare-import path without claiming packed-install or `--preserve-symlinks` compatibility.
- After T004b integrates, T006 (`protocol-client`) and T007 (`relay-room`) may be leased concurrently because their locks and write scopes are disjoint; their approved commits still integrate serially with root gates after each.
