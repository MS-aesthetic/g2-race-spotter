# Implementation plan — 2026-09-03T14:43:23-04:00
Status: BUILDING
Current spec focus: specs/010-monorepo-bootstrap.md

## Next (ordered; the worker takes the first unchecked task)
- [ ] T002f (owner: hud-qa) (spec: 010 AC-7) Repair T002e by replacing the fixed-date no-report assertion with a recursive assertion that no `report.json` exists anywhere beneath the harness-selected output root for both launcher and ping failure paths; verify `scripts/test/sim-harness.test.ts` plus the root test/typecheck/lint gates.
  - Reviewer finding (verbatim): "[block] scripts/test/sim-harness.test.ts:21 — the no-report assertion hard-codes `2026-09-03` instead of proving that the harness-selected output root contains no report — scenario: on any later date, a failed launch writes a partial `qa/<current-date>/sim/report.json` → both tests inspect only `qa/2026-09-03/sim/report.json` and pass while AC-7 is violated — fix: assert recursively that no `report.json` exists beneath `selectedOutputRoot`, or inject the report path/date used by the harness and assert that exact path is absent."
...
## Done this cycle
- (none; T002e is review-blocked at commit 6f3b4b3 and remains unchecked beneath repair T002f)

## Notes / why
- Iteration 15 records T002e as `review-blocked`/`block`: the CLI seams are present, but the fixed-date assertion cannot prove absence of a partial report beneath the selected output root on later dates.
- T002f stays first and quotes the blocking finding verbatim; T002e and its original T002b remain unchecked beneath it. This is the second consecutive block in the AC-7 repair chain, not the third-block human-escalation threshold.
...
- AC audit 020: AC-1–AC-9 are unmet because their named verifier files are absent and the package/relay code is still placeholder-only; AC-10 is open `[HW]`, with local CLI prerequisite T014.
- AC audit 030: AC-1–AC-6 are unmet because their named tests and feature implementations are absent; AC-7 is open `[SIM]`; AC-8–AC-10 are open `[HW]`.
- AC audit 040: AC-1–AC-5 are unmet because their named tests and UI implementation are absent; AC-6–AC-7 are open `[HW]`.
- AC audit 050: AC-1–AC-5 are unmet because their named tests and image renderer are absent; AC-5b is open `[SIM]`; AC-6–AC-8 are open `[HW]`.
- AC audit 060: AC-1–AC-4 are unmet because their named tests and hardening implementations are absent; AC-5–AC-8 are open `[HW]`.
- AC audit 070: AC-1–AC-3 are unmet because their named tests and packaging artifacts are absent; AC-4–AC-5 are open `[HW]`.
