...
|---|---|---|---|---|
| L007 | T007a | active from `a380d6f` | T007/T007b integrated; only `services/relay/src/index.ts`, `services/relay/src/worker-configuration.d.ts`, `services/relay/wrangler.jsonc`, `services/relay/test/routes.test.ts`, and `services/relay/src/race-room.ts` solely for the internal debug-state path/RPC required by the gated Worker route; lock `relay-http` | fresh `relay-backend-dev`; exact `a380d6f..HEAD` review by `protocol-keeper`; integrate only an approved commit, then run the live route test and every Node 22 root gate |
