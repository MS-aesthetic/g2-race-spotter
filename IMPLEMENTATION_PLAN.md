# Implementation plan — 2026-09-03T22:37:15-04:00
...
| ------ | ---------------- | ----------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| L008R  | T008             | relay-room; `191d9a1`                           | `services/relay/src/race-room.ts`, `services/relay/test/replay.test.ts`, `services/relay/test/alarm.test.ts`; lock `relay-room`              | T007a/T007c integrated; coordinator records the isolated worktree before dispatch; fresh `protocol-keeper` reviews exact `191d9a1..HEAD`; integrate only after approval, both live tests, and every Node 22 root gate |
...
- [ ] T008 (owner: relay-backend-dev) (spec: 020 AC-4; requirement R4) [lease: L008R; stream: relay-room; lock: relay-room; base: `191d9a1`] Preserve authoritative state and monotonic sequence across driver reconnect and Durable Object rehydration; when the final socket leaves, repoint the single alarm to `updatedAt + ROOM_TTL_MS`. Add live coverage in `services/relay/test/replay.test.ts` and `services/relay/test/alarm.test.ts`; run both tests and every root gate.
...
- The approved T007a/T007c stack closes local R5 plumbing but does not claim deployed AC-10; T102 remains `[HW]`.
- L007R is retired. L008R is the only runnable relay-room lease because T011/T009/T012/T013 share its lock and depend serially on prior room semantics.
- T014 stays unleased until T008 and T009 integrate because replay and driver-eviction scenarios depend on them.
- T006 remains frozen after three blocked reviews; T008 can proceed without human approval, but AC-8 needs explicit authorization before a fourth client repair.
- Integration evidence at `191d9a1`: 56 tests, typecheck, lint/format, pin checks, agent-sync checks, live relay tests, Wrangler type generation check, and a dry-run with temporary ignored assets all passed on Node 22; main is clean.
- AC audit 010: AC-1/AC-2/AC-6/AC-7 met; AC-3 and AC-5 parked under Needs simulator; AC-4 under Needs human [HW].
- AC audit 020: AC-1/AC-2/AC-3 met; R5 local routing is met; AC-4–AC-9 remain unmet except quarantined AC-8; AC-10 remains [HW] with local scenario prerequisite T014.
