...
- Reconnect: handled by the shared `RoomClient` — on `open` it sends `hello`, takes the replayed `state` as truth, then flushes only intents queued *while the socket was down* (latest `lane`, latest `cars` triple, any `msg`/`clear` in order). Car taps made before the session has replayed are **not** sent as a triple built from the pre-drop local copy: `main.ts` holds the tapped rows and, on the first `state` of the session, sends `rebaseCars(state.cars, pendingRows)` — only the tapped rows override the replayed room, so rows the relay cleared meanwhile (the 6 s stale clear) do not come back. Never re-send an already-delivered `msg` (each `msg` gets a new id and would duplicate). Show the banner until the first `state` arrives, then reconcile the UI to it.
